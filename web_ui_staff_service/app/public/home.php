                ...
                <div class="modal-body">
                    ...
                    <div id="detailsContainer">
                        <div class="row">
                            ...
                            <div class="col-auto">
                                <button type="button" class="btn btn-primary" id="addDetailBtn">Add More Rundown</button>
                            </div>
                        </div>
    ...
    <script>
        $(document).ready(function() {
                ...
                success: function(response) {
                    if (response.success) {
                        ...
                        for (var i = 0; i < staffList.length; i++) {
                            ...
                            selectLocation.append('<option value="" hidden>Select a PIC staff</option>');

                            // Add staff names and IDs as options
                            for (var j = 0; j < staffArray.length; j++) {
                                var staffItem = staffArray[j];
                                selectLocation.append('<option value="' + staffItem.id + '">' + staffItem.name + '</option>');
                            }
                        }
            ...
            `;
                ...
                for (var j = 0; j < staffArray.length; j++) {
                    var staffItem = staffArray[j];
                    selectLocation.innerHTML += '<option value="' + staffItem.id + '">' + staffItem.name + '</option>';
                }
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        if (response.success) {
                            ...
                            var accordionContainer = $('<div class="accordion" id="orderAccordion"></div>');
                            ...
                            for (var i = 0; i < orderDetails.length; i++) {
                                ...
                                var itemHeader = $('<h2 class="accordion-header" id="heading' + i + '"></h2>');
                                itemHeader.append('<button class="accordion-button collapsed" type="button" data-bs-toggle="collapse" data-bs-target="#collapse' + i + '" aria-expanded="false" aria-controls="collapse' + i + '">Day: ' + (i + 1) + '</button>');

                                // Create the item content
                                var itemContent = $('<div id="collapse' + i + '" class="accordion-collapse collapse" aria-labelledby="heading' + i + '"></div>');
                                ...
                                rightColumn.append('<p>Time Start: ' + detail.time_start + '</p>');
                                ...
                                var editButton = $('<div class="col-md-4"><button style="width: 100%" id="' + detail.id + '" class="btn btn-primary btn-sm" type="button">Add Event</button></div>');
